<template>
  <div class="h-screen">
    ...
    <div v-else class="relative flex flex-col overflow-hidden p-5 xl:h-[100vh]">
      <Location class="relative flex h-[7vh] flex-row border border-indigo-600 xl:h-[15vh]">
        ...
        <div class="hidden w-[10rem] items-center border-r border-indigo-600 xl:flex xl:w-[25rem] xl:text-[1.2rem]">
          <div class="ml-5 text-indigo-600">{{ currentTime }}</div>
        </div>
        <div class="flex w-[10rem] items-center justify-center border-indigo-600 md:border-r xl:w-[13rem] xl:text-[2rem]">
          <div class="text-indigo-600">{{ currentTem }}°C</div>
        </div>
        <div class="flex w-full flex-row items-center justify-center">
          <div class="hidden overflow-hidden md:flex md:w-4/6">
            <div class="marquee text-bold text-[1rem] text-indigo-600 xl:text-[2rem]">
              <span>AFTER READING IT, GO WASH AND SLEEP</span>
              <span>AFTER READING IT, GO WASH AND SLEEP</span>
              <span>AFTER READING IT, GO WASH AND SLEEP</span>
            </div>
          </div>
          <div class="flex h-full w-full flex-row items-center justify-center border-l border-indigo-600 md:w-2/6">
            <a href="https://suzuru.vercel.app/" class="flex text-2xl font-extrabold text-indigo-600">
              其餘作品
            </a>
          </div>
        </div>
      ...
      <inner-box class="flex h-[40vh] flex-row xl:h-[80vh]">
        <bubble
          ...
          <div class="absolute animate-pulse text-[1.3rem] font-extrabold text-white xl:text-[4.5rem]">
            TAKE IT OUT TOMORROW
          </div>
        </bubble>
        <information class="hidden w-3/6 flex-col xl:flex">
          <div class="relative flex h-3/6 flex-row items-center justify-center border-x border-indigo-600">
            <div class="absolute top-3 left-3">
              <video class="w-[5rem] xl:w-[10rem]" :src="bubbleGif" autoplay loop muted></video>
            </div>
            <video class="h-full w-screen" :src="weatherGif" autoplay loop muted></video>
            <div class="absolute top-3 text-[1.3rem] font-extrabold text-indigo-600 xl:text-[1.5rem]">
              IMAGERY OF WEATHER CONDITIONS
            </div>
          </div>
          <div class="flex h-2/6 flex-col items-center justify-center border border-indigo-600">
            <div class="xt-2xl fontte-extrabold float-left text-indigo-600 xl:text-4xl">
              明日預報
            </div>
            <div>
              <div class="flex items-center font-mono text-indigo-600 xl:flex-row">
          ...
          <div class="flex h-1/6 flex-row items-center justify-center border-x border-indigo-600 text-indigo-600">
      ...
      <information class="relative flex h-auto w-full flex-col xl:hidden">
        <div class="flex h-4/6 flex-col items-center justify-center border border-indigo-600 py-6">
          <div class="float-left text-3xl font-extrabold text-indigo-600 xl:text-4xl">
            明日預報
          </div>
          <div class="items-start justify-start pt-10">
            <div class="flex w-full flex-col items-center font-mono text-indigo-600">
              <div class="pr-3 text-2xl font-semibold">-9A.M-</div>
              <div class="py-3 text-xl">
                溫度：{{ temperature9am }}°C,天氣狀態：{{ description9am }}
              </div>
            </div>
            <div class="flex flex-col items-center font-mono text-indigo-600 xl:flex-row">
      ...
      <UpdateTime class="relative flex h-[4vh] h-auto flex-row xl:h-[5vh]">
        <div class="flex w-3/12 items-center justify-center bg-indigo-600 py-2 font-mono text-sm text-white xl:w-1/12">
          更新時間
        </div>
        <div
